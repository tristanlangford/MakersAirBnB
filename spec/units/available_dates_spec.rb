...
describe Available_dates do
  ...
  it('Should initialize with start/end date and prop_id') do
    expect(subject.start_date).to eq('01/01/2020')
    expect(subject.end_date).to eq('10/01/2020')
  ...
  describe('List_date') do
    it('Should return an instance of Available_dates with all info') do
      ...
      expect(Available_dates.list_date(1).start_date).to eq('01/01/2020')
      expect(Available_dates.list_date(1).end_date).to eq('10/01/2020')
      expect(Available_dates.list_date(1).prop_id).to eq('1')
    end 
  end
