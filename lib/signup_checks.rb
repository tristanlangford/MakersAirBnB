...
class SignupChecks
  ...
  def self.sign_in_checks(email, password)
    ...
    return :passwords_do_not_match if !encrypted_password_match(email, password)
  end
  ...
  def self.encrypted_password_match(email, password)
    BCrypt::Password.new(get_password_from_db(email)) == password
  end
end
